...
export class SignComponent implements OnInit {
  ...
  signTypeChange() {
    ...
    switch (this.signTypeSelected) {
      ...
      case this.signTypes[0]:
        this.walletAccount = this.accounts.find(a => a.id.replace('brl_', 'xbrl_') === this.signatureAccount);
  ...
  async prepareTransaction() {
    ...
    if (this.txType === TxType.send || this.txType === TxType.change) {
      this.signatureAccount = this.fromAccountID.replace('brl_', 'xbrl_').toLowerCase();
    } else if (this.txType === TxType.receive || this.txType === TxType.open) {
      this.signatureAccount = this.toAccountID.replace('brl_', 'xbrl_').toLowerCase();
    }
