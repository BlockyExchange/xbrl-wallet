...
export class AccountDetailsComponent implements OnInit, OnDestroy {
  ...
  async generateSend() {
    ...
    if (!this.validateAmount()) return this.notifications.sendWarning(`Invalid XBRL Amount`);
    ...
    if (from.balanceBN.minus(rawAmount).lessThan(0)) return this.notifications.sendError(`From account does not have enough XBRL`);
    ...
    const blockData = {
      account: this.accountID.replace('brl_', 'xbrl_').toLowerCase(),
    ...
    const blockDataPrevious = {
      account: jsonBlock.account.replace('brl_', 'xbrl_').toLowerCase(),
