...
export class MultisigComponent implements OnInit {
  ...
  addAccount() {
    ...
    if (this.storedAccounts.includes(this.accountAdd.replace('brl_', 'xbrl_').toLocaleLowerCase())) {
    ...
    this.storedAccounts.push(this.accountAdd.replace('brl_', 'xbrl_').toLocaleLowerCase());
    this.accountAdd = '';
