...
export class QrModalComponent implements OnInit {
  ...
  nano_scheme = /^(xrb|nano|nanorep|nanoseed|nanokey|brl|xbrl):.+$/g;
  ...
  onCodeResult(resultString: string) {
    ...
    if (this.util.account.isValidAccount(resultString)) {
      ...
      if (['nano:', 'nanorep:', 'xrb:', 'brl:', 'xbrl:'].includes(url.protocol) && this.util.account.isValidAccount(url.pathname)) {
        type = 'account';
